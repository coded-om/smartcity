"""
app.py — Flask application factory and startup orchestration.

Responsibilities (and only these):
  • Create Flask app, register CORS and blueprints
  • Expose legacy /  and /data routes for backward compatibility
  • Run one-time startup tasks (DB init, device seeding, auto-train, FR loops)
  • Launch MQTT daemon thread
...
# config must be imported first — its module-level code populates os.environ
# from the .env file before any other local module reads os.getenv().
...
def _auto_train_all() -> None:
    """Auto-train models for devices that have enough data but no model yet."""
    conn    = db.get_db()
...
def _start_face_recognition_for_cameras() -> None:
    """Launch FR loops for all cameras that have it enabled."""
    ...
    for cam in cameras:
        try:
            ...
            print(f"🔍 FR loop started for camera {cam['id']}")
        except Exception as exc:
            print(f"⚠️  FR loop error for camera {cam['id']}: {exc}")
...
# ── YOLO warm-up ──────────────────────────────────────────────────────────────
# Load the YOLOv8 model into memory now so the first detection request
# doesn't pay the model-load penalty at runtime.
threading.Thread(target=object_detector.warmup, daemon=True).start()

# ── Periodic Isolation Forest retraining (APScheduler) ───────────────────────
# Retrains all device models every RETRAIN_INTERVAL_HOURS hours so the
# anomaly detector adapts to shifting sensor baselines automatically.
...
_scheduler.start()
print(f"⏰ Periodic retrain scheduler started (every {_RETRAIN_INTERVAL_HOURS}h)")
